...
class HeuristicGrade(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.thresholdPoints = [int(t) for t in testDict['thresholdPoints'].split()]
...
#                points += 1
#Extra
        for z in range(len(self.thresholds)):
            if expanded <= self.thresholds[z]:
                points += self.thresholdPoints[z]
#Extra Ends
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        #EXTRA, USED TO CONFIGURE DYNAMIC THRESHOLD VALUES
        thresholdPoints = [int(x) for x in solutionDict['thresholdPoints'].split()]
        game_state = pacman.GameState()
        lay = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        game_state.initialize(lay, 0)
        problem = searchAgents.CornersProblem(game_state)
